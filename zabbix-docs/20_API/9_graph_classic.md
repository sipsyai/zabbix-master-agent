---
title: Graph (classic)
source: https://www.zabbix.com/documentation/current/en/manual/api/reference/dashboard/widget_fields/graph_classic
downloaded: 2025-11-14 10:40:48
---
...
These parameters and the possible property values for the respective dashboard widget field objects allow to configure the [_Graph (classic)_](/documentation/current/en/manual/web_interface/frontend_sections/dashboards/widgets/graph_classic) widget in `dashboard.create` and `dashboard.update` methods.

Widget `fields` properties are not validated during the creation or update of a dashboard. This allows users to modify [built-in widgets](/documentation/current/en/manual/web_interface/frontend_sections/dashboards/widgets) and create [custom widgets](/documentation/current/en/devel/modules/tutorials/widget), but also introduces the risk of creating or updating widgets incorrectly. To ensure the successful creation or update of the _Graph (classic)_ widget, please refer to the parameter behavior outlined in the tables below.

### Parameters

The following parameters are supported for the _Graph (classic)_ widget.
...
900 - 15 minutes.  
---|---|---|---  
_Source_ | 0 | source_type | 0 - _(default)_ Graph;  
1 - Simple graph.  
_Graph_ | 6 | graphid.0 | [Graph](/documentation/current/en/manual/api/reference/graph/get) ID.  
  
[Parameter behavior](/documentation/current/en/manual/api/reference_commentary#parameter-behavior):  
\- _required_ if _Source_ is set to "Graph"  
| _Graph (Widget)_ | 1 | graphid._reference | Instead of [Graph](/documentation/current/en/manual/api/reference/graph/get) ID:  
`ABCDE._graphid` \- set a [compatible widget](/documentation/current/en/manual/web_interface/frontend_sections/dashboards/widgets#widget-compatibility) (with its _Reference_ parameter set to "`ABCDE`") as the data source for graphs.  
  
[Parameter behavior](/documentation/current/en/manual/api/reference_commentary#parameter-behavior):  
\- _required_ if _Source_ is set to "Simple graph" and _Graph_ is not set  
_Item_ | 4 | itemid.0 | [Item](/documentation/current/en/manual/api/reference/item/get) ID.  
  
[Parameter behavior](/documentation/current/en/manual/api/reference_commentary#parameter-behavior):  
\- _required_ if _Source_ is set to "Simple graph" and _Item (Widget)_ is not set  
...
[Parameter behavior](/documentation/current/en/manual/api/reference_commentary#parameter-behavior):  
\- _required_ if _Source_ is set to "Simple graph" and _Item_ is not set  
...
The following examples aim to only describe the configuration of the dashboard widget field objects for the _Graph (classic)_ widget. For more information on configuring a dashboard, see [`dashboard.create`](/documentation/current/en/manual/api/reference/dashboard/create).

#### Configuring a _Graph (classic)_ widget

Configure a _Graph (classic)_ widget that displays a simple graph for the item "42269".
